...
try:
    ...
    from music_downloader import MusicDownloader, DownloadException, DownloadResult, AudioFormat
            ...
            try:
                ...
                if not download_result.success:
                    api_service.logger.warning(
                        f"歌曲 {mid} 下载失败，将跳过: {download_result.error_message}"
                    )
                results.append(download_result)
            except DownloadException as e:
                api_service.logger.warning(f"歌曲 {mid} 下载异常: {e}")
                results.append(DownloadResult(success=False, error_message=str(e)))
            time.sleep(0.6)
